...
void Frame::RemoveOutliers(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera)
{
    ...
    if(mvKeys.empty())
        return;
     //Delete kp
     std::vector <cv::KeyPoint> _mvkeys;//暂存除去人标签的特征点
     cv::Mat _mDescriptors;//暂存除去人标签的描述子
    
    //若特征点落在人标签的区域内，则标志赋为true，将非人标签特征点放入暂存变量中
    for (unsigned int i=0; i < mvKeys.size(); i++)
    {
        bool bool_key = false;  // 初始化一个布尔变量，用于标记当前关键点是否在人区域内
        cv::KeyPoint orb_kp = mvKeys[i];   // 获取当前遍历到的关键点
        for(auto vit_area = mvPersonArea.begin(); vit_area != mvPersonArea.end(); vit_area++)  // 遍历mvPersonArea容器中的所有元素
        {
            if (vit_area->contains(orb_kp.pt))  // 检查关键点是否在感兴趣区域(vit_area)内
            {
                bool_key = true;  // 如果关键点在人区域内，将bool_key设置为true并跳出循环
                break;
            }
        }
        if(bool_key == false)
...
{
    if(mDistCoef.at<float>(0)!=0.0)  // 检查畸变系数的第一个元素是否不为0
    {
        cv::Mat mat(4,2,CV_32F);  // 创建一个4x2的浮点型矩阵
        mat.at<float>(0,0)=0.0; mat.at<float>(0,1)=0.0;
        mat.at<float>(1,0)=imLeft.cols; mat.at<float>(1,1)=0.0;
        mat.at<float>(2,0)=0.0; mat.at<float>(2,1)=imLeft.rows;
        mat.at<float>(3,0)=imLeft.cols; mat.at<float>(3,1)=imLeft.rows;

        mat=mat.reshape(2);   // 将矩阵重塑为2列
        cv::undistortPoints(mat,mat,static_cast<Pinhole*>(mpCamera)->toK(),mDistCoef,cv::Mat(),mK);  // 对矩阵进行去畸变处理
        mat=mat.reshape(1);   // 将矩阵重塑为1列

        // Undistort corners   // 计算去畸变后的四个角点的最小和最大x、y坐标
        mnMinX = min(mat.at<float>(0,0),mat.at<float>(2,0));
        mnMaxX = max(mat.at<float>(1,0),mat.at<float>(3,0));
        mnMinY = min(mat.at<float>(0,1),mat.at<float>(1,1));
        mnMaxY = max(mat.at<float>(2,1),mat.at<float>(3,1));
    }
    else   // 如果畸变系数的第一个元素为0，则直接使用原始图像的尺寸作为边界
    {
        mnMinX = 0.0f;
        mnMaxX = imLeft.cols;
        mnMinY = 0.0f;
...
void Frame::SetYoloResults(vector<cv::Rect2i> &result, int i){
    if(i==0)
    {
        mvPersonArea = result;
    }
    if(i==1)
    {
        mvDynamicArea = result;
    }
}

void Frame::set_stuff_keypoints(std::vector<cv::KeyPoint> keypoints) {stuff_keypoints = keypoints;}

void Frame::set_stuff_descriptor(cv::Mat descriptor) {stuff_descriptor = descriptor;}
