...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, Settings* settings, const string &_nameSeq):
    mState(NO_IMAGES_YET), mSensor(sensor), mTrackedFr(0), mbStep(false),mbNewYoloFlag(false),
...
void Tracking::GetImg(const cv::Mat& img)
{
    ...
    mpDetector -> mbImagFlag = true;
    img.copyTo(mpDetector->mImage);
}

bool Tracking::IsNewYoloImgArrived()
{
    std::unique_lock<mutex> lock(mpDetector->mMutexImgYolo);
    if(mbNewYoloFlag)
    {
        mbNewYoloFlag = false;
        return true;
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)  //抓取RGB-D 图片
{
    mImGray = imRGB;
    cv::Mat imDepth = imD;  
    ...
    while (!IsNewYoloImgArrived())
        {
            usleep(1);
        }
    mvDynamicArea = mpDetector->mvDynamicArea;
    mCurrentFrame.SetYoloResults(mpDetector->mvPersonArea,0);
    mCurrentFrame.SetYoloResults(mpDetector->mvDynamicArea,1);
    ...
    {   
        ...
        curr_thing_descriptor.release();
        Thing_Stuff_Detachment(mCurrentFrame.mvKeys,mCurrentFrame.mDescriptors); 
        ...
        {  
            ...
            last_thing_descriptor.release();
            curr_thing_keypoints.begin()和curr_thing_keypoints.end()）。
            last_thing_keypoints.insert(last_thing_keypoints.end(),curr_thing_keypoints.begin(),curr_thing_keypoints.end());//填充信息
            last_thing_descriptor = curr_thing_descriptor;
        }
    ...
    else 
    {
        ...
        last_thing_keypoints.insert(last_thing_keypoints.end(),curr_thing_keypoints.begin(),curr_thing_keypoints.end());//保存第一帧相关特征点信息
        last_thing_descriptor = curr_thing_descriptor;
    }
...
void Tracking::Thing_Stuff_Detachment(std::vector<cv::KeyPoint> mvKeys,cv::Mat mDescriptors)
{
    ...
    for (unsigned int i=0; i < mvKeys.size(); i++)
    {
        ...
        {
            if (vit_area->contains(orb_kp.pt)) 
            {
                bool_key = true;  
                break;  
            }
        ...
        if(bool_key == false)
        {
            ...
            filtered_stuff_key_points.push_back(mvKeys[i]);
            filtered_stuff_descriptor.push_back(mDescriptors.row(i));
        }
        else
        {
            curr_thing_keypoints.push_back(mvKeys[i]);
            curr_thing_descriptor.push_back(mDescriptors.row(i));
        }
    }
...
{                                           
    ...
    std::vector<cv::KeyPoint>  curr_keypoints = curr_thing_keypoints;
    cv::Mat curr_descriptor = curr_thing_descriptor;
      
            

    
    std::vector<cv::KeyPoint> last_keypoints = last_thing_keypoints;
    cv::Mat last_descriptor = last_thing_descriptor;
