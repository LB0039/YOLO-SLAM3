    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    mpDetector = new YoloDetection();  // 创建一个YoloDetection对象，并将其指针赋值给mpDetector
    // 创建一个新的线程，该线程将执行ORB_SLAM3::YoloDetection类的Run方法，并将mpDetector作为参数传递
    // mptDetector是一个指向thread对象的指针
    mptDetector = new thread(&ORB_SLAM3::YoloDetection::Run, mpDetector);
    ...
    mpDetector->SetTracking(mpTracker);
...
{
    if(mSensor!=RGBD  && mSensor!=IMU_RGBD)  // 检查传感器类型是否为RGBD或IMU_RGBD
    {
        cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;  // 如果传感器类型不是RGBD或IMU_RGBD，输出错误信息并退出程序
        exit(-1);
    }

    cv::Mat imToFeed = im.clone();  // 克隆原始图像，创建一个新的Mat对象imToFeed，用于后续处理
    cv::Mat imDepthToFeed = depthmap.clone();
    if(settings_ && settings_->needToResize()){  // 检查是否需要调整图像大小
        cv::Mat resizedIm;  // 创建一个用于存储调整大小后的图像的Mat对象
    ...
    // Check mode change
    {
        unique_lock<mutex> lock(mMutexMode);  // 创建一个互斥锁，确保在同一时间只有一个线程可以访问以下代码块
        if(mbActivateLocalizationMode)
        {
            mpLocalMapper->RequestStop();
            ...
            while(!mpLocalMapper->isStopped())
            {
                usleep(1000);  // 暂停1毫秒，避免过度占用CPU资源
            }

            mpTracker->InformOnlyTracking(true);  // 通知跟踪器仅进行跟踪，不进行定位
            mbActivateLocalizationMode = false;   // 设置标志位，表示已停用定位模式
        }
        if(mbDeactivateLocalizationMode)
        {
            mpTracker->InformOnlyTracking(false);
